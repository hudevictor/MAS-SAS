                    ...
                    std::set<size_t>& hashmap, float threshold, int maxNumOps)
{
  ...
  //start
   // bool flagq=true;
    float acceptProbability=0.55;  //0.55
  ...
  if (depth >= (int)srcOps.size()) {
    ...
    } else if((int)newGraph->inEdges.size() < maxNumOps){//&&newGraph->total_cost() < graph->total_cost()
          float randValue = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
         // printf("randValue=%f\n",randValue);
           if (hashmap.find(newGraph->hash()) == hashmap.end()&&randValue < acceptProbability){//&&candidates.empty()
