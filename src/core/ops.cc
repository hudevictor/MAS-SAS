...
Graph* Graph::optimize(float alpha, int budget, bool print_subst)
{
  ...
  //for(int i= 0; i < strlen(taso_path) ; i++)

    //printf("%c", taso_path[i]);
...
*/
//s
inline uint64_t microsecond_timer1() {
...
Graph* Graph::optimize(float alpha, int budget, bool print_subst)
{
  ...
  const char* taso_path ="/home/user1/x/taso1";
  std::string graph_subst_file = std::string(taso_path)+ "/graph_subst.pb";
  printf("graph_subst_file=%s",graph_subst_file.c_str());
  GraphXfer::load_graph_xfer_from_pb_file(model, xfers, graph_subst_file);
  ...
  std::priority_queue<Graph*, std::vector<Graph*>, GraphCompare> remains;
  ...
  int counter = 0;
  int maxNumOps = inEdges.size();
  long long start_time = microsecond_timer1();
  ofstream timer_fs;
  timer_fs.open("timer2.txt");
  printf("\n        ===== Start Cost-Based Local Search =====\n");
...
for (int iter = 0; iter < budget; ++iter) {
    ...
    if (counter % 1 == 0) {
      printf("        [%d] cost = %.4lf bestCost = %.7lf candidates.size() = %zu\n", counter, subGraph->total_cost(), bestCost, candidates.size());
     // if (timer_fs.is_open()) {
      timer_fs << microsecond_timer1() - start_time << ", " << bestCost << std::endl;
      //printf("aaaaaaaaaaaaaaaa\n");
      //}else{
       // std::cout << "无法打开文件" << std::endl;
      //}
      //printf("t=%lld\n",microsecond_timer1() - start_time);
    }
    counter ++;

        // 贪心阶段：尝试找到成本更低的图  
         // printf("sizeofxfers=%zu\n",xfers.size());
        
    for (size_t i = 0; i < xfers.size(); i++) { 
               //   int cv=0;
       //start 清空之前的，保持candiates只有本次规则的候选
           while (!candidates.empty()) {
             // if(cv<1){
                remains.push(candidates.top());
            //  }
            //  cv++;
                 candidates.pop();
              }     
...
//e
 ...
 timer_fs.close();
  bestGraph = bestGraph->preprocess_weights();
  printf("        ===== Finish Cost-Based Local Search =====\n\n");
  //printf("bestCost = %.4lf\n", bestGraph->total_cost());
  printf("Optimized graph: end-to-end execution time =\n");
  printf("%.8lf ms (average of 100 runs)\n", bestGraph->run());
  bestGraph->print_costs();
...
void Graph::print_costs(void)
{
  ...
  printf("        Cost metrics: exe_time(%.7lf) flops(%.4lf) "
